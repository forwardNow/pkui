...
var Swiper = function (selector, params) {
    ...
    function onTouchStart(event) {
        ...
        // FIX 某些元素不可拖动
        if ( params.releaseElementsClass ) {
            var releaseElementsClass,
                $eventTarget
                ;
            releaseElementsClass = params.releaseElementsClass;
            if ( releaseElementsClass.indexOf( "." ) !== 0 ) {
                releaseElementsClass = "." + releaseElementsClass;
            }
            $eventTarget = jQuery( eventTarget );
            if ( $eventTarget.is( releaseElementsClass ) || $eventTarget.parents( releaseElementsClass ).size() > 0) {
